...
function saveOAuthState(state, verifier) {
  ...
  try { localStorage.setItem(VERIFIER_KEY, verifier); } catch {}
  try { sessionStorage.setItem(VERIFIER_KEY, verifier); } catch {}
}

function getOAuthState() {
  const state = localStorage.getItem(STATE_KEY) || sessionStorage.getItem(STATE_KEY) || '';
...
function writeAuth(auth) {
  ...
  try { localStorage.setItem(TOKEN_KEY, json); } catch {}
  try { sessionStorage.setItem(TOKEN_KEY, json); } catch {}
}
...
async function ensureFirebaseConfig() {
  if (window.SADE_FIREBASE_CONFIG?.projectId && !String(window.SADE_FIREBASE_CONFIG.projectId).startsWith('YOUR_')) {
    return window.SADE_FIREBASE_CONFIG;
  }
  const existing = document.querySelector('script[data-sade-firebase-config]');
  if (existing) {
    ...
    await new Promise((resolve, reject) => {
      ...
      script.dataset.sadeFirebaseConfig = 'true';
      script.onload = resolve;
      script.onerror = () => reject(new Error('SADE Firebase configuration could not be loaded.'));
      document.head.appendChild(script);
    });
  }
  const config = window.SADE_FIREBASE_CONFIG;
  if (!config?.projectId || String(config.projectId).startsWith('YOUR_')) {
    throw new Error('SADE Firebase is not configured. Add the real Firebase project configuration before connecting GitHub.');
  }
  return config;
}
...
async function beginWebAuth(clientId) {
...
async function exchangeCode(code, verifier) {
  ...
  const endpoint = `https://europe-west2-${config.projectId}.cloudfunctions.net/githubOAuthExchange`;
  ...
  if (!response.ok || data.error || !data.access_token) {
    throw new Error(data.error || `GitHub OAuth exchange failed (${response.status}).`);
  }
  return data;
}
...
async function completeWebAuth() {
  ...
  // Immediately strip code, state, and error params from the URL address bar to prevent stale callback retries on reload
  if (typeof window !== 'undefined' && window.history && window.history.replaceState) {
    const cleanUrl = window.location.pathname + window.location.hash;
    window.history.replaceState({}, document.title, cleanUrl || CALLBACK_URL);
  }

  if (oauthError) {
    clearOAuthState();
    throw new Error(query.get('error_description') || `GitHub authorization failed: ${oauthError}`);
  }

  const { state: expectedState, verifier } = getOAuthState();
  clearOAuthState();
...
const callbackPromise = completeWebAuth().catch(error => {
  clearOAuthState();
  console.error('SADE GitHub OAuth callback', error);
  window.SADE_GITHUB_OAUTH_ERROR = error.message;
});
...
async function connect(onProgress) {
  ...
  onProgress?.({ type: 'redirect' });
  await beginWebAuth(clientId);
  return null;
}
