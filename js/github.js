...
async function connect(onProgress){const clientId=configuredClientId();if(!clientId)throw new Error('SADE needs the GitHub App Client ID before it can connect.');const device=await requestDeviceCode(clientId);onProgress?.({type:'device',...device});const verification=device.verification_uri||device.verification_uri_complete;if(verification)window.open(verification,'_blank','noopener,noreferrer');const token=await pollDeviceToken(clientId,device.device_code,device.interval,message=>onProgress?.({type:'pending',message}));writeAuth(normaliseToken(token));return getCurrentUser();}
async function refreshAccessToken(){const auth=readAuth();const clientId=configuredClientId();if(!auth?.refreshToken||!clientId)return null;const token=await oauthJson('/login/oauth/access_token',{client_id:clientId,refresh_token:auth.refreshToken,grant_type:'refresh_token'}).catch(()=>null);if(!token?.access_token)return null;const refreshed=normaliseToken(token);writeAuth(refreshed);return refreshed;}
async function ensureToken(){let auth=readAuth();if(!auth?.accessToken)throw new Error('GitHub is not connected.');if(auth.expiresAt&&Date.now()>auth.expiresAt-60000){auth=await refreshAccessToken();if(!auth){clearAuth();throw new Error('The GitHub session expired. Please reconnect.');}}return auth.accessToken;}
