(() => {
  ...
  function renderResult(result) {
    const output = result?.output || 'SADE returned no engineering analysis.';
    addMessage(output, 'sade');
    const evidence = result?.evidence;
    if (evidence) {
      addMessage(`LIVE EVIDENCE\nFiles inspected: ${evidence.filesInspected?.length ?? 0}\nTree entries: ${evidence.treeCount ?? 0}\nTree truncated: ${evidence.truncated ? 'yes' : 'no'}`, 'sade');
    }
    if (result?.audit) addMessage(`AUDIT\n${JSON.stringify(result.audit, null, 2)}`, 'sade');
    if (result?.validation) addMessage(`VALIDATION\n${JSON.stringify(result.validation, null, 2)}`, 'sade');
    if (result?.patches) addMessage(`PROPOSED PATCHES\n${JSON.stringify(result.patches, null, 2)}`, 'sade');
  }
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      const result = await runRealEngineering(objective);
      renderResult(result);
    } catch (error) {
